...
export function listSlackAccountIds(cfg: AlizeConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: AlizeConfig): string {
...
function mergeSlackAccountConfig(cfg: AlizeConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: AlizeConfig): ResolvedSlackAccount[] {
