...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "alize.sandbox=1");
  args.push("--label", `alize.sessionKey=${params.scopeKey}`);
  args.push("--label", `alize.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `alize.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "alize.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`alize sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`alize sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("alize sandbox recreate --all");
}
