...
function resolveMainSessionKeyForSandbox(params: { cfg?: AlizeConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: AlizeConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  lines.push(
    `- See: ${formatCliCommand(`alize sandbox explain --session ${runtime.sessionKey}`)}`,
  );
