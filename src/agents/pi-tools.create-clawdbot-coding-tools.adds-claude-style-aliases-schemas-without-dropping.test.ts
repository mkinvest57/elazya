...
import { __testing, createAlizeCodingTools } from "./pi-tools.js";
...
describe("createAlizeCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAlizeCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alize-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "alize-outside.txt");
