...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.ALIZE_AGENT_DIR;
    else process.env.ALIZE_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alize-chutes-"));
    process.env.ALIZE_STATE_DIR = tempDir;
    process.env.ALIZE_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
