...
import { createAlizeCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("alize-ws-", async (workspaceDir) => {
      await withTempDir("alize-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createAlizeCodingTools({ workspaceDir });
    ...
    await withTempDir("alize-ws-", async (workspaceDir) => {
      await withTempDir("alize-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello alize");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("alize-ws-", async (workspaceDir) => {
      await withTempDir("alize-override-", async (overrideDir) => {
        const tools = createAlizeCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("alize-sandbox-", async (sandboxDir) => {
      await withTempDir("alize-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "alize-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "alize-sandbox:bookworm-slim",
            containerPrefix: "alize-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createAlizeCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
