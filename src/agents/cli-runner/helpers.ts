...
/**
 * Cleanup suspended Alizé CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: AlizeConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alize-cli-images-"));
  const paths: string[] = [];
