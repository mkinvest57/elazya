...
const defaultTools = createAlizeCodingTools();

describe("createAlizeCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alize-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createAlizeCodingTools();
    ...
    try {
      ...
      const contents = "Hello from alize read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "alize-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "alize-workspace"),
    ...
    const tools = createAlizeCodingTools({ sandbox });
