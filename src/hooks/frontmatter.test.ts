...
`;
    ...
    expect(parsed.alize.emoji).toBe("💾");
    expect(parsed.alize.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.alize.emoji).toBe("📝");
    expect(parsed.alize.events).toEqual(["command"]);
    expect(parsed.alize.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.alize.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"alize": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"alize": {"events": ["test"]}}');
  });
...
describe("resolveAlizeMetadata", () => {
  it("extracts alize metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAlizeMetadata(frontmatter);
  ...
  it("returns undefined when alize key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        alize: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Alizé" },
            { id: "npm", kind: "npm", package: "@alize/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@alize/hook");
  });
    ...
    "alize":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Alizé" }],
      },
  }
---
...
`;
    ...
    const alize = resolveAlizeMetadata(frontmatter);
    expect(alize).toBeDefined();
    expect(alize?.emoji).toBe("💾");
    expect(alize?.events).toEqual(["command:new"]);
    expect(alize?.requires?.config).toEqual(["workspace.dir"]);
    expect(alize?.install?.[0].kind).toBe("bundled");
  });
