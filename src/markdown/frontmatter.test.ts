...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { alize?: { emoji?: string } };
    expect(parsed.alize?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { alize?: { events?: string[] } };
