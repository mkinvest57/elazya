...
export const WIDE_AREA_DISCOVERY_DOMAIN = "alize.internal.";
export const WIDE_AREA_ZONE_FILENAME = "alize.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*alize-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "alize";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "alize");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "alize-gw");
  ...
  records.push(`_alize-gw._tcp IN PTR ${instanceLabel}._alize-gw._tcp`);
  records.push(`${instanceLabel}._alize-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._alize-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; alize-content-hash: ${contentHash}\n${contentBody}`;
}
