...
function candidateBinDirs(opts: EnsureAlizePathOpts): string[] {
  ...
  // Bundled macOS app: `alize` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingAlizé = path.join(execDir, "alize");
    if (isExecutable(siblingAlizé)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/alize` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "alize"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `alize` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAlizeCliOnPath(opts: EnsureAlizePathOpts = {}) {
  if (isTruthyEnvValue(process.env.ALIZE_PATH_BOOTSTRAPPED)) return;
  process.env.ALIZE_PATH_BOOTSTRAPPED = "1";
