...
export function triggerAlizeRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ALIZE_SYSTEMD_UNIT,
        process.env.ALIZE_PROFILE,
      );
  ...
  const label =
    process.env.ALIZE_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ALIZE_PROFILE);
