...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alize-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "alize doctor",
        managerScriptArgs(manager, "alize", ["doctor", "--non-interactive"]),
        gitRoot,
        { ALIZE_UPDATE_IN_PROGRESS: "1" },
      ),
    );
