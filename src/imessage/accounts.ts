...
export function listIMessageAccountIds(cfg: AlizeConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: AlizeConfig): string {
...
function mergeIMessageAccountConfig(cfg: AlizeConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: AlizeConfig): ResolvedIMessageAccount[] {
