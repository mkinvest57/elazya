...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.alizeCanvasA2UIAction ||
        window.webkit.messageHandlers.alizeCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.alizeCanvasA2UIAction &&
        typeof window.alizeCanvasA2UIAction.postMessage === "function") ||
      (window.alizeCanvasA2UIAction &&
        typeof window.alizeCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.alizeSendUserAction === "function" ? window.alizeSendUserAction : undefined;
  if (!window.alizeSendUserAction && legacySend) {
    window.alizeSendUserAction = legacySend;
  }
  if (!window.alizeSendUserAction && typeof window.alizeSendUserAction === "function") {
    window.alizeSendUserAction = window.alizeSendUserAction;
  }
  const hasHelper = () =>
    typeof window.alizeSendUserAction === "function" ||
    typeof window.alizeSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("alize:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Alizé node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.alizeSendUserAction === "function"
        ? window.alizeSendUserAction
        : window.alizeSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ALIZE_SKIP_CANVAS_HOST)) return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Alizé Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
