...
const requireAgentDir = () => {
  const agentDir = process.env.ALIZE_AGENT_DIR;
  if (!agentDir) throw new Error("ALIZE_AGENT_DIR not set");
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.ALIZE_STATE_DIR;
  const previousAgentDir = process.env.ALIZE_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.ALIZE_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.ALIZE_AGENT_DIR;
    } else {
      process.env.ALIZE_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.ALIZE_OAUTH_DIR;
  });

  it("writes auth-profiles.json under ALIZE_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alize-oauth-"));
    process.env.ALIZE_STATE_DIR = tempStateDir;
    process.env.ALIZE_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ALIZE_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ALIZE_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alize-minimax-"));
    process.env.ALIZE_STATE_DIR = tempStateDir;
    process.env.ALIZE_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ALIZE_AGENT_DIR;
