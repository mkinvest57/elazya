...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidate = path.resolve(root, entry.name, ".alize");
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".alize");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.alize out of git; it contains credentials and session history.",
