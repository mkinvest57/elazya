...
function resolveConfiguredModelRaw(cfg: AlizeConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: AlizeConfig): string[] {
...
export function applyPrimaryModel(cfg: AlizeConfig, model: string): AlizeConfig {
...
export function applyModelAllowlist(cfg: AlizeConfig, models: string[]): AlizeConfig {
  const defaults = cfg.agents?.defaults;
