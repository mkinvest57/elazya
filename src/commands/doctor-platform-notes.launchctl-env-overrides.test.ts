...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ALIZE_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ALIZE_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ALIZE_GATEWAY_TOKEN");
    expect(message).not.toContain("ALIZE_GATEWAY_PASSWORD");
  });
