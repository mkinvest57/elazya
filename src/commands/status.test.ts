...
beforeAll(() => {
  previousProfile = process.env.ALIZE_PROFILE;
  process.env.ALIZE_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.ALIZE_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/alize-root.js", () => ({
  resolveAlizePackageRoot: vi.fn().mockResolvedValue("/tmp/alize"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/alize/pnpm-lock.yaml",
      markerPath: "/tmp/alize/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Alizé status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("alize status --all") ||
          l.includes("alize --profile isolated status --all") ||
          l.includes("alize status --all") ||
          l.includes("alize --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ALIZE_GATEWAY_TOKEN = "abcd1234";
    try {
