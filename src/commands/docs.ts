...
const SEARCH_TOOL = "https://docs.molt.bot/mcp.SearchAlizé";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    ...
    if (isRich()) {
      ...
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('alize docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.molt.bot/");
      runtime.log(`Search: ${formatCliCommand('alize docs "your query"')}`);
    }
    return;
  }
