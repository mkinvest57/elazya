...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Alizé status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("alize security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("alize security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("alize status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("alize logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("alize status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("alize gateway probe")}`);
  }
}
