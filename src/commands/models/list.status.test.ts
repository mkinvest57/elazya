...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAlizeAgentDir: vi.fn().mockReturnValue("/tmp/alize-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAlizeAgentDir: mocks.resolveAlizeAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/alize-agent/auth-profiles.json");
