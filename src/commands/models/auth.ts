...
import { readConfigFileSnapshot, type AlizeConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: AlizeConfig, model: string): AlizeConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("alize plugins install")}\`.`,
    );
  }
