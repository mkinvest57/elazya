...
function stripUnknownConfigKeys(config: AlizeConfig): {
  config: AlizeConfig;
  removed: string[];
} {
  const parsed = AlizeSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as AlizeConfig;
...
function noteOpencodeProviderOverrides(cfg: AlizeConfig) {
...
function hasExplicitConfigPath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.ALIZE_CONFIG_PATH?.trim() || env.ALIZE_CONFIG_PATH?.trim());
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (!hasExplicitConfigPath(process.env) && snapshot.exists) {
    ...
    if (basename === "alize.json") {
      const canonicalPath = path.join(path.dirname(snapshot.path), "alize.json");
  ...
  let cfg: AlizeConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as AlizeConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("alize doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("alize doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "alize doctor --fix" to remove these keys.');
    }
  }
