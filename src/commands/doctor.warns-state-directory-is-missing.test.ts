...
beforeEach(() => {
  ...
  resolveAlizePackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ALIZE_STATE_DIR;
  originalUpdateInProgress = process.env.ALIZE_UPDATE_IN_PROGRESS;
  process.env.ALIZE_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "alize-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ALIZE_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ALIZE_UPDATE_IN_PROGRESS;
  } else {
    process.env.ALIZE_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveAlizePackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadAlizePlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/alize-root.js", () => ({
  resolveAlizePackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "alize-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.ALIZE_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when ALIZE_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/alize.json",
      exists: true,
      raw: "{}",
    ...
    process.env.ALIZE_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
