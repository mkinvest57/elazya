...
function resolveDefaultAccountId(cfg: AlizeConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: AlizeConfig): boolean {
...
export function summarizeBindings(cfg: AlizeConfig, bindings: AgentBinding[]): string[] {
