...
import type { AlizeConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AlizeConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("alize security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("alize configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("alize config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("alize doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "alize config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("alize config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
