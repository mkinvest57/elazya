...
export function listAgentEntries(cfg: AlizeConfig): AgentEntry[] {
...
function resolveAgentName(cfg: AlizeConfig, agentId: string) {
...
function resolveAgentModel(cfg: AlizeConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AlizeConfig): AgentSummary[] {
