...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.ALIZE_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.ALIZE_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alize-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.ALIZE_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.ALIZE_STATE_DIR = stateDir;
    process.env.ALIZE_CONFIG_PATH = path.join(stateDir, "alize.json");
