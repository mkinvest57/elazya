...
import { discoverAlizePlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { AlizePackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  alize?: AlizePackageManifest;
  [LEGACY_MANIFEST_KEY]?: AlizePackageManifest;
};
...
const ENV_CATALOG_PATHS = ["ALIZE_PLUGIN_CATALOG_PATHS", "ALIZE_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<AlizePackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageAlizé?: AlizePackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageAlizé;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.alize ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageAlizé: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverAlizePlugins({ workspaceDir: options.workspaceDir });
