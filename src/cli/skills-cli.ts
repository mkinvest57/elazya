...
function appendAlizeHubHint(output: string, json?: boolean): string {
  if (json) return output;
  return `${output}\n\nTip: use \`npx alizehub\` to search, install, and sync skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("alize skills list")}\` to see all skills.`
      : "No skills found.";
    return appendAlizeHubHint(message, opts.json);
  }
  ...
  return appendAlizeHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendAlizeHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("alize skills list")}\` to see available skills.`,
      opts.json,
    );
  }
