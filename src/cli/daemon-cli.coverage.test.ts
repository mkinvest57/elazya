...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ALIZE_STATE_DIR: process.env.ALIZE_STATE_DIR,
    ALIZE_CONFIG_PATH: process.env.ALIZE_CONFIG_PATH,
    ALIZE_GATEWAY_PORT: process.env.ALIZE_GATEWAY_PORT,
    ALIZE_PROFILE: process.env.ALIZE_PROFILE,
  };

  beforeEach(() => {
    process.env.ALIZE_STATE_DIR = "/tmp/alize-cli-state";
    process.env.ALIZE_CONFIG_PATH = "/tmp/alize-cli-state/alize.json";
    delete process.env.ALIZE_GATEWAY_PORT;
    delete process.env.ALIZE_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.ALIZE_STATE_DIR !== undefined)
      process.env.ALIZE_STATE_DIR = originalEnv.ALIZE_STATE_DIR;
    else delete process.env.ALIZE_STATE_DIR;

    if (originalEnv.ALIZE_CONFIG_PATH !== undefined)
      process.env.ALIZE_CONFIG_PATH = originalEnv.ALIZE_CONFIG_PATH;
    else delete process.env.ALIZE_CONFIG_PATH;

    if (originalEnv.ALIZE_GATEWAY_PORT !== undefined)
      process.env.ALIZE_GATEWAY_PORT = originalEnv.ALIZE_GATEWAY_PORT;
    else delete process.env.ALIZE_GATEWAY_PORT;

    if (originalEnv.ALIZE_PROFILE !== undefined)
      process.env.ALIZE_PROFILE = originalEnv.ALIZE_PROFILE;
    else delete process.env.ALIZE_PROFILE;
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ALIZE_PROFILE: "dev",
        ALIZE_STATE_DIR: "/tmp/alize-daemon-state",
        ALIZE_CONFIG_PATH: "/tmp/alize-daemon-state/alize.json",
        ALIZE_GATEWAY_PORT: "19001",
      },
