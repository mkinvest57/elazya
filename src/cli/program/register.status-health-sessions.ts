...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["alize status", "Show channel health + session summary."],
          ["alize status --all", "Full diagnosis (read-only)."],
          ["alize status --json", "Machine-readable output."],
          ["alize status --usage", "Show model provider usage/quota snapshots."],
          [
            "alize status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["alize status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
  ...
  program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["alize sessions", "List all sessions."],
          ["alize sessions --active 120", "Only last 2 hours."],
          ["alize sessions --json", "Machine-readable output."],
          ["alize sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
