...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "alize-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      ALIZE_NO_RESPAWN: "1",
      ALIZE_STATE_DIR: stateDir,
      ALIZE_SKIP_CHANNELS: "1",
      ALIZE_SKIP_GMAIL_WATCHER: "1",
      ALIZE_SKIP_CRON: "1",
      ALIZE_SKIP_BROWSER_CONTROL_SERVER: "1",
      ALIZE_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "alize-entry-bootstrap.mjs");
