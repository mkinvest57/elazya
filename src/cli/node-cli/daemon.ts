...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("alize node install"), formatCliCommand("alize node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("alize node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ALIZE_LOG_PREFIX: baseEnv.ALIZE_LOG_PREFIX ?? "node",
