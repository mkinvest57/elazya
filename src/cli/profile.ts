...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.alize${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.ALIZE_PROFILE = profile;

  const stateDir = env.ALIZE_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.ALIZE_STATE_DIR?.trim()) env.ALIZE_STATE_DIR = stateDir;

  if (!env.ALIZE_CONFIG_PATH?.trim()) {
    env.ALIZE_CONFIG_PATH = path.join(stateDir, "alize.json");
  }

  if (profile === "dev" && !env.ALIZE_GATEWAY_PORT?.trim()) {
    env.ALIZE_GATEWAY_PORT = "19001";
  }
}
