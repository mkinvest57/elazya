...
import { resolveAlizePackageRoot } from "../infra/alize-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "alize doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "alize";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "alize"]);
const CLI_NAME = resolveCliName();
const ALIZE_REPO_URL = "https://github.com/alize/alize.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "alize");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("alize@")) return trimmed.slice("alize@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.ALIZE_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ALIZE_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ALIZE_GIT_DIR points at a non-git directory: ${params.dir}. Set ALIZE_GIT_DIR to an empty folder or a alize checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ALIZE_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Alizé update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Alizé..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Alizé install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("alize doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("alize gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g alize@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g alize@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.ALIZE_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.ALIZE_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("alize gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("alize doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("alize gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("alize gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `alize update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `ALIZE_GIT_DIR points at a non-git directory: ${gitDir}. Set ALIZE_GIT_DIR to an empty folder or a alize checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via ALIZE_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Alizé to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["alize update", "Update a source checkout (git)"],
        ["alize update --channel beta", "Switch to beta channel (git + npm)"],
        ["alize update --channel dev", "Switch to dev channel (git + npm)"],
        ["alize update --tag beta", "One-off update to a dist-tag or version"],
        ["alize update --no-restart", "Update without restarting the service"],
        ["alize update --json", "Output result as JSON"],
        ["alize update --yes", "Non-interactive (accept downgrade prompts)"],
        ["alize update wizard", "Interactive update wizard"],
        ["alize --update", "Shorthand for alize update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run alize update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.molt.bot/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["alize update status", "Show channel + version status."],
          ["alize update status --json", "JSON output."],
          ["alize update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
