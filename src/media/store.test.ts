...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "ALIZE_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "alize-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ALIZE_STATE_DIR = path.join(home, ".alize");
    ...
    await fs.mkdir(path.join(home, ".alize"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.alize${path.sep}media`);
