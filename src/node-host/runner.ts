...
const execHostEnforced = process.env.ALIZE_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.ALIZE_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureAlizeCliOnPath({ pathEnv: process.env.PATH ?? "" });
