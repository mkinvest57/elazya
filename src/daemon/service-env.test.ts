...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ALIZE_GATEWAY_PORT).toBe("18789");
    expect(env.ALIZE_GATEWAY_TOKEN).toBe("secret");
    expect(env.ALIZE_SERVICE_MARKER).toBe("alize");
    expect(env.ALIZE_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ALIZE_SERVICE_VERSION).toBe("string");
    expect(env.ALIZE_SYSTEMD_UNIT).toBe("alize-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ALIZE_LAUNCHD_LABEL).toBe("bot.molt.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ALIZE_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ALIZE_SYSTEMD_UNIT).toBe("alize-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ALIZE_LAUNCHD_LABEL).toBe("bot.molt.work");
    }
  });
});
