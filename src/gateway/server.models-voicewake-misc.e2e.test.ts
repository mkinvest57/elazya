...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.ALIZE_STATE_DIR = path.join(homeDir, ".alize");
    ...
    return () => {
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.ALIZE_STATE_DIR = prevStateDir;
      }
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "alize-home-"));
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".alize", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.ALIZE_CANVAS_HOST_PORT;
    process.env.ALIZE_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.ALIZE_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevToken === undefined) {
      ...
      process.env.ALIZE_GATEWAY_TOKEN = prevToken;
    }
    if (prevCanvasPort === undefined) {
      delete process.env.ALIZE_CANVAS_HOST_PORT;
    } else {
      process.env.ALIZE_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.ALIZE_CONFIG_PATH;
    if (!configPath) throw new Error("Missing ALIZE_CONFIG_PATH");
