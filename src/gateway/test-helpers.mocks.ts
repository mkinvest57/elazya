...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `alize-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.ALIZE_CONFIG_PATH = path.join(root, "alize.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "alize.json");
