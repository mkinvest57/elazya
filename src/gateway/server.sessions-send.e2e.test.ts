...
import { createAlizeTools } from "../agents/alize-tools.js";
...
beforeAll(async () => {
  prevGatewayPort = process.env.ALIZE_GATEWAY_PORT;
  prevGatewayToken = process.env.ALIZE_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.ALIZE_GATEWAY_PORT = String(gatewayPort);
  process.env.ALIZE_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayPort === undefined) {
    ...
    process.env.ALIZE_GATEWAY_PORT = prevGatewayPort;
  }
  if (prevGatewayToken === undefined) {
    delete process.env.ALIZE_GATEWAY_TOKEN;
  } else {
    process.env.ALIZE_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createAlizeTools().find((candidate) => candidate.name === "sessions_send");
