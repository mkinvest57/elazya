...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.ALIZE_STATE_DIR;
  previousConfigPath = process.env.ALIZE_CONFIG_PATH;
  previousSkipBrowserControl = process.env.ALIZE_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.ALIZE_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.ALIZE_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alize-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.ALIZE_SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "alize-test-"))
    : path.join(tempHome, ".alize-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.ALIZE_STATE_DIR;
    else process.env.ALIZE_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.ALIZE_CONFIG_PATH;
    else process.env.ALIZE_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.ALIZE_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.ALIZE_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.ALIZE_SKIP_GMAIL_WATCHER;
    else process.env.ALIZE_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.ALIZE_SKIP_CANVAS_HOST;
    else process.env.ALIZE_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.ALIZE_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ALIZE_GATEWAY_TOKEN = fallbackToken;
  }
