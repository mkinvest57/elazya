...
import { resolveAlizeAgentDir } from "../agents/agent-paths.js";
...
import { ensureAlizeModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { AlizeConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ALIZE_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ALIZE_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ALIZE_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<AlizeConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.ALIZE_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.ALIZE_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveAlizeAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alize-live-state-"));
  process.env.ALIZE_STATE_DIR = tempStateDir;
  ...
  process.env.ALIZE_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.alize-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAlizeAgentDir();
  const sanitizedCfg: AlizeConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Alizé live tool probe (local, safe): " +
    ...
    process.env.ALIZE_CONFIG_PATH = previous.configPath;
    process.env.ALIZE_GATEWAY_TOKEN = previous.token;
    process.env.ALIZE_SKIP_CHANNELS = previous.skipChannels;
    process.env.ALIZE_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ALIZE_SKIP_CRON = previous.skipCron;
    process.env.ALIZE_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ALIZE_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ALIZE_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureAlizeModelsJson(cfg);
      ...
      const rawModels = process.env.ALIZE_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.alize-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
