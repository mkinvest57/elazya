...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Alizé";
  if (/alize/i.test(trimmed)) return trimmed;
  return `${trimmed} (Alizé)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.ALIZE_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "alize");
  ...
  const binCli = path.join(cwd, "bin", "alize.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.ALIZE_TAILNET_DNS?.trim();
