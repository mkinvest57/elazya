...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-alize-message-channel") ?? "");
  const accountId = getHeader(req, "x-alize-account-id")?.trim() || undefined;
  ...
  const allTools = createAlizeTools({
