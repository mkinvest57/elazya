...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.ALIZE_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alize-gw-mock-home-"));
      ...
      const toolProbePath = path.join(workspaceDir, `.alize-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".alize");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "alize.json");
      ...
      process.env.ALIZE_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.ALIZE_CONFIG_PATH = prev.configPath;
        process.env.ALIZE_GATEWAY_TOKEN = prev.token;
        process.env.ALIZE_SKIP_CHANNELS = prev.skipChannels;
        process.env.ALIZE_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.ALIZE_SKIP_CRON = prev.skipCron;
        process.env.ALIZE_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.ALIZE_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alize-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.ALIZE_STATE_DIR = prev.stateDir;
      process.env.ALIZE_CONFIG_PATH = prev.configPath;
      process.env.ALIZE_GATEWAY_TOKEN = prev.token;
      process.env.ALIZE_SKIP_CHANNELS = prev.skipChannels;
      process.env.ALIZE_SKIP_GMAIL_WATCHER = prev.skipGmail;
