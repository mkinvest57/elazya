import type { AlizeConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: AlizeConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AlizeConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AlizeConfig),
...
export function getCommandDetection(_cfg?: AlizeConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AlizeConfig) {
