...
import { scheduleGatewaySigusr1Restart, triggerAlizeRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting alize in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAlizeRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting alize via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
