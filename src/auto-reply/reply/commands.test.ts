...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "alize-commands-"));
...
function buildParams(commandBody: string, cfg: AlizeConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
