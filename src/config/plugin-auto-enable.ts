import type { AlizeConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: AlizeConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: AlizeConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: AlizeConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: AlizeConfig): boolean {
...
function isIMessageConfigured(cfg: AlizeConfig): boolean {
...
function isWhatsAppConfigured(cfg: AlizeConfig): boolean {
...
function isGenericChannelConfigured(cfg: AlizeConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: AlizeConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AlizeConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: AlizeConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: AlizeConfig, pluginId: string): boolean {
...
function enablePluginEntry(cfg: AlizeConfig, pluginId: string): AlizeConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
