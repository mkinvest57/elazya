...
const ETC_ALIZE_DIR = path.join(ROOT_DIR, "etc", "alize");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "alize.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcAlizePath(...parts: string[]) {
  return path.join(ETC_ALIZE_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcAlizePath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "alize.json"))).toEqual({
      shared: true,
    });
  });
});
