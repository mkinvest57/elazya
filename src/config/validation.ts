...
import type { AlizeConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AlizeConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: AlizeConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AlizeSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AlizeConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AlizeConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as AlizeConfig)),
    ),
  };
}
