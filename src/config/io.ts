...
import type { AlizeConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareAlizeVersions } from "./version.js";
...
function coerceConfig(value: unknown): AlizeConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AlizeConfig;
}
...
function stampConfigVersion(cfg: AlizeConfig): AlizeConfig {
...
function warnIfConfigFromFuture(cfg: AlizeConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareAlizeVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Alizé (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: AlizeConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AlizeConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as AlizeConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AlizeConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AlizeConfig) {
    clearConfigCache();
...
// module scope. `ALIZE_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ALIZE_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ALIZE_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): AlizeConfig {
...
export async function writeConfigFile(cfg: AlizeConfig): Promise<void> {
  clearConfigCache();
