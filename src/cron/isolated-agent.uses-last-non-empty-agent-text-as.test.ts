...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "alize-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".alize", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AlizeConfig> = {},
): AlizeConfig {
  const base: AlizeConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".alize", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
