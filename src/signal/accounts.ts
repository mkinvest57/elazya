...
export function listSignalAccountIds(cfg: AlizeConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: AlizeConfig): string {
...
function mergeSignalAccountConfig(cfg: AlizeConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: AlizeConfig): ResolvedSignalAccount[] {
