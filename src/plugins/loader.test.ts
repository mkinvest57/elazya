...
import { loadAlizePlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.ALIZE_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `alize-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "alize.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ALIZE_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadAlizePlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.ALIZE_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadAlizePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAlizePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@alize/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        alize: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
