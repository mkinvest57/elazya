...
import { discoverAlizePlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AlizePluginDefinition;
  register?: AlizePluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AlizePluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AlizePluginDefinition;
...
export function loadAlizePlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAlizePlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "alize/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AlizePluginModule | null = null;
    try {
      mod = jiti(candidate.source) as AlizePluginModule;
    } catch (err) {
