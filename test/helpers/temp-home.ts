...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ALIZE_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.ALIZE_STATE_DIR = path.join(base, ".alize");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "alize-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".alize", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
