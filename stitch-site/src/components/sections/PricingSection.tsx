  ...
  return (
    <section id="pricing" className="py-24 scroll-mt-20">
      <motion.div 
        ...
        whileInView="show"
        viewport={{ once: true }}
        className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 px-4"
      >
        {/* PLAN SOLO */}
        <motion.div variants={item} className="bg-white border border-gray-200 rounded-[32px] p-10 flex flex-col hover:shadow-sm transition-shadow">
          ...
          <ul className="space-y-4 mb-12 flex-1">
            {["10 agents Core", "Morning Briefing auto", "Pilotage Telegram", "100% Local"].map((feature, i) => (
              <li key={i} className="flex items-start gap-3 text-base text-[#373a46] font-geist font-medium">
        ...
        {/* PLAN PRO */}
        <motion.div variants={item} className="bg-[#fcfcfc] border border-gray-300 md:scale-105 rounded-[32px] p-10 flex flex-col relative z-10 shadow-2xl">
          ...
          <ul className="space-y-4 mb-12 flex-1">
            <li className="font-geist font-medium text-[#0f172a] border-b border-gray-200 pb-4 mb-2 flex items-center gap-3">
              <Check className="w-5 h-5 text-[#0f172a] shrink-0" />
              Tout le Solo
            </li>
          ...
          <button className="w-full py-4 rounded-full font-geist font-medium text-white bg-gradient-to-b from-gray-800 to-black shadow-[inset_-4px_-6px_25px_0px_rgba(201,201,201,0.08),inset_4px_4px_10px_0px_rgba(29,29,29,0.24)] hover:scale-105 transition-all duration-300">
            Acheter Pro
          </button>
        </motion.div>
        ...
        <motion.div variants={item} className="bg-white border border-gray-200 rounded-[32px] p-10 flex flex-col hover:shadow-sm transition-shadow">
          ...
          <ul className="space-y-4 mb-12 flex-1">
            <li className="font-geist font-medium text-[#0f172a] border-b border-gray-100 pb-4 mb-2 flex items-center gap-3">
              <Check className="w-5 h-5 text-[#0f172a] shrink-0" />
              Tout le Pro
            </li>
            {["4 agents coordonnés en équipe", "Health Monitoring hebdo", "Créateur d'agents visuel", "Multi-utilisateurs (2-5 pers)"].map((feature, i) => (
              <li key={i} className="flex items-start gap-3 text-base text-[#373a46] font-geist font-medium">
                <Check className="w-5 h-5 text-gray-400 shrink-0" />
                {feature}
              </li>
