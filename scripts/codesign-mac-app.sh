...
ENT_TMP_APP_BASE=$(mktemp -t elazya-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t elazya-entitlements-runtime.XXXXXX)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/Elazya" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/Elazya" "$APP_ENTITLEMENTS"
fi
